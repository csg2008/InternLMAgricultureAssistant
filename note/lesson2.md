...
## 七、试用遇到的问题与解决方法
> - 刷新页面就会导致模型重复加载一次，显存超出后页面异常，重启进程后浏览器又会自动刷新就进入死循环。只能在出错时关闭浏览器，再重启进程，最后打开浏览器继续试用，这个问题对初学者不太友好
> - 书生浦语7B对问题意图捕捉不稳定，有时可以正常回答，有时就是完全不相关的回复
> - 在启用数据分析插件时好几次遇到死循环现象，一直在重复同一个操作不退出，只能主动停止后问个不相关的问题再重试
> - 在启用数据分析插件时要求以图表的方式展示数据就无响应，重试过好多次都这样，不知道是什么原因
> - 在启用数据分析插件时插件对输出数据格式指令遵循不够严谨，容易多输出不数据，不知道是什么原因